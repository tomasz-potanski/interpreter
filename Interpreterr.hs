                                ...
                                NonEmptyArgs (DLList ((Ident identArg):_) typee) ->
                                    if not(isAFunctionType(typee)) then
                                        ...
                                        if not (funcDeclTypeOK funcDeclLine typee) then
                                            ...
                                            in
                                            let stateAfterFunctionCall = showToUser ("#DEBUG, newDecl: " ++ (show funcDeclLine)) (interpretStmt stmt (addOneFunction2 newDeclLine ((M.union tStateOld extState) , funcMap)))
                                            in
